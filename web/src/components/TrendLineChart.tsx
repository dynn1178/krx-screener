...
/** 거래대금은 편차가 몇십~몇백 배라 로그축이 아니면 상위 1개가 나머지를 눌러 평평해진다.
 * 1억원 미만은 로그축에서 의미가 없어 바닥으로 눌러 log10 을 안전하게 만든다. */
const LOG_FLOOR = 1e8;
const logExpTicks = (minExp: number, maxExp: number): number[] => {
  const lo = Math.ceil(minExp), hi = Math.floor(maxExp);
  const out: number[] = [];
  for (let e = lo; e <= hi; e++) out.push(Math.pow(10, e));
  return out.length ? out : [Math.pow(10, Math.round((minExp + maxExp) / 2))];
};
...
export default function TrendLineChart({
  ...
  /** 거래대금은 로그, 등락률은 선형 — y 매핑 직전에만 적용해서 나머지 로직은 원래 값을 그대로 쓴다 */
  const scaleV = (v: number) =>
    metric === "tradeValue" ? Math.log10(Math.max(v, LOG_FLOOR)) : v;

  const { yMin, yMax } = useMemo(() => {
    const vals: number[] = [];
    visible.forEach((s) => s.points.forEach((p) => {
      if (metric === "tradeValue" && p.mentions === 0) return;
      const v = valueOf(p);
      if (v != null && Number.isFinite(v)) vals.push(scaleV(v));
    }));
  ...
  const y = (v: number) => innerH - ((scaleV(v) - yMin) / (yMax - yMin || 1)) * innerH;

  const yTicks = metric === "tradeValue" ? logExpTicks(yMin, yMax) : niceTicks(yMin, yMax, 4);
